...
import { CollectionApiActions, CollectionPageActions, SelectedBookPageActions } from '@client/books/actions';
...
export class CollectionEffects {
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.storageService
          .getCollection()
          .pipe(
            map((books: Book[]) => CollectionApiActions.loadBooksSuccess({ books })),
            catchError((error) => of(CollectionApiActions.loadBooksFailure({ error })))
          )
      )
    )
  );
  ...
  constructor(private actions$: Actions, private storageService: BookStorageService) {}
}
