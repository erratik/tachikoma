...
import { BookActions } from '@client/books/actions';
...
export class BookExistsGuard implements CanActivate {
  constructor(private store: Store<fromBooks.State>, private googleBooks: GoogleBooksService, private router: Router) {}
  ...
  waitForCollectionToLoad(): Observable<boolean> {
    return this.store.pipe(select(fromBooks.getCollectionLoaded), filter((loaded) => loaded), take(1));
  }
  ...
  hasBookInStore(id: string): Observable<boolean> {
    return this.store.pipe(select(fromBooks.getBookEntities), map((entities) => !!entities[id]), take(1));
  }
  ...
  canActivate(route: ActivatedRouteSnapshot): Observable<boolean> {
    return this.waitForCollectionToLoad().pipe(switchMap(() => this.hasBook(route.params.id)));
  }
}
